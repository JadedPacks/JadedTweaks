...
public class BloodMagicHelper {
	...
	public static void AlchemyRecipe() {
		if(Loader.isModLoaded("exnihilo")) {
			AlchemyRecipeRegistry.registerRecipe(Parts.spawnChicken, 2, new ItemStack[] {
				Parts.flesh, Parts.flesh, Parts.diamond, Parts.feather, Parts.leather
			}, 2);
			AlchemyRecipeRegistry.registerRecipe(Parts.spawnCow, 2, new ItemStack[] {
				Parts.leather, Parts.leather, Parts.diamond, Parts.leather
			}, 2);
			AlchemyRecipeRegistry.registerRecipe(Parts.soulshard, 2, new ItemStack[] {
				Parts.bloodshard, Parts.bloodshard, Parts.lifeshard, Parts.bloodshard
			}, 2);
		}
		AlchemyRecipeRegistry.registerRecipe(Parts.blazerod, 2, new ItemStack[] {
			Parts.blazepowder, Parts.blazepowder, Parts.blazepowder, Parts.blazepowder
		}, 2);

		if(Parts.hearts != null) {
			AlchemyRecipeRegistry.registerRecipe(Parts.hearts, 2, new ItemStack[] {
				Parts.flesh, Parts.flesh, Parts.bone, Parts.diamond
			}, 2);
		}
	...
	private static void addRecipes() {
		if(Loader.isModLoaded("exnihilo")) {
			...
			GameRegistry.addRecipe(new ShapedOreRecipe(Parts.knife, "GGG", "XFG", "FXG", 'F', Parts.flint, 'G', "blockGlass"));
			if(Parts.searedbrick != null) {
				GameRegistry.addRecipe(new ShapedOreRecipe(Parts.altar, "SXS", "SFS", "TLT", 'S', "stone", 'F', Parts.furnace, 'T', Parts.searedbrick, 'L', Parts.flint));
			}
		}
