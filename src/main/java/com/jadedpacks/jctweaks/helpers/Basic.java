...
public class Basic {
	public static void init() {
		...
		GameRegistry.addRecipe(new ShapedOreRecipe(new ItemStack(Blocks.cobblestone), "C", "C", 'C', Parts.cobbleslab));
		if(Loader.isModLoaded("Natura") && Loader.isModLoaded("harvestcraft")) {
			GameRegistry.addRecipe(new ShapedOreRecipe(Parts.topiary, "XSX", "SDS", "XSX", 'D', Parts.dirt, 'S', "listAllseed"));
		}
		if(OreDictionary.getOres("oreTin").size() > 0) {
			GameRegistry.addRecipe(new ShapedOreRecipe(new ItemStack(Items.bucket), "IXI", "XIX", 'I', "ingotTin"));
			Main.log.info("Alternative Bucket Recipe tin loaded");
			GameRegistry.addRecipe(new ShapedOreRecipe(Parts.piston, "PPP", "CIC", "CRC", 'I', "ingotTin", 'P', "plankWood", 'C', "cobblestone", 'R', "dustRedstone"));
		}
		if(OreDictionary.getOres("oreAluminum").size() > 0) {
			GameRegistry.addRecipe(new ShapedOreRecipe(new ItemStack(Items.bucket), "IXI", "XIX", 'I', "ingotAluminum"));
			Main.log.info("Alternative Bucket Recipe aluminum loaded");
			GameRegistry.addRecipe(new ShapedOreRecipe(Parts.piston, "PPP", "CIC", "CRC", 'I', "ingotAluminum", 'P', "plankWood", 'C', "cobblestone", 'R', "dustRedstone"));
		} else if(OreDictionary.getOres("oreAluminium").size() > 0) {
			GameRegistry.addRecipe(new ShapedOreRecipe(new ItemStack(Items.bucket), "IXI", "XIX", 'I', "ingotAluminium"));
			Main.log.info("Alternative Bucket aluminium Recipe loaded");
			GameRegistry.addRecipe(new ShapedOreRecipe(Parts.piston, "PPP", "CIC", "CRC", 'I', "ingotAluminium", 'P', "plankWood", 'C', "cobblestone", 'R', "dustRedstone"));
		}
		if(OreDictionary.getOres("oreCopper").size() > 0) {
			GameRegistry.addRecipe(new ShapedOreRecipe(new ItemStack(Items.bucket), "IXI", "XIX", 'I', "ingotCopper"));
		...
		if(Loader.isModLoaded("ExtraUtilities") && Loader.isModLoaded("exnihilo")) {
			...
			GameRegistry.addRecipe(new ShapedOreRecipe(builderswand, "XXD", "XDX", "OXX", 'D', "gemDiamond", 'O', "obsidian"));
			Main.log.info("Jaded added a recipe for the builders wand");
		}
		if(Loader.isModLoaded("customnpcs") && Loader.isModLoaded("Thaumcraft")) {
			GameRegistry.addRecipe(new ShapedOreRecipe(Parts.artifact, "DXG", "ASR", 'D', Parts.demoningot, 'G', Parts.goldcoin, 'A', Parts.amethyst, 'S', Parts.saphire, 'R', Parts.ruby));
		}
		...
		if(Loader.isModLoaded("progressiveautomation") && Loader.isModLoaded("exnihilo")) {
			...
			GameRegistry.addRecipe(new ShapedOreRecipe((cobblegen), "SSS", "LMW", "SSS", 'S', "stone", 'L', lava, 'M', miner, 'W', "listAllwater"));
			...
			GameRegistry.addRecipe(new ShapedOreRecipe((stonegen), "SES", "SRS", "SSS", 'S', "stone", 'E', rfengine, 'R', "dustRedstone"));
		}
		if(Loader.isModLoaded("AgriCraft") && Loader.isModLoaded("exnihilo")) {
			...
			GameRegistry.addRecipe(new ShapedOreRecipe(journal, "CSC", "SWS", "CSC", 'C', cropsticks, 'S', "listAllseed", 'W', "plankWood"));
		...
		if(Loader.isModLoaded("MoCreatures") && Loader.isModLoaded("ThermalExpansion")) {
			if(Parts.silversword1 != null && (Parts.silversword2 != null)) {
				GameRegistry.addRecipe(Parts.silversword1, "XSX", 'S', Parts.silversword2);
			}
		}
		if(OreDictionary.getOres("treeLeaves").size() > 0) {
			GameRegistry.addRecipe(new ShapedOreRecipe(Parts.podzolitem, "LLL", "LDL", "LLL", 'D', Parts.dirt, 'L', "treeLeaves"));
		}
	}
}